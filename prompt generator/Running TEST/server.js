...
function createServer() {
  ...
  if (req.method === "OPTIONS" && pathname === "/api/recommend") {
    res.writeHead(204, {
  ...
  if (pathname === "/") {
    serveStatic(res, path.join(ROOT, "Running sam.html"));
    return;
  }

  const publicPath = path.normalize(path.join(ROOT, pathname));
...
function startServer(port = PORT) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, () => {
      ...
      console.log(`Running TEST server is listening on http://localhost:${actualPort}`);
